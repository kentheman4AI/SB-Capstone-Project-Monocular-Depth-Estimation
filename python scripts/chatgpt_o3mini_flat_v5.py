...
def random_base_quad(img_shape, painting_shape, scale_range=(0.2, 0.5)):
    """
    ...
    return base_quad, (target_h, target_w)

def random_perturb_quad(img_shape, target_loc, base_quad, perturbation=0.1):
    """
    Randomly perturb the destination quadrilateral.
    """
    img_h, img_w = img_shape[:2]
    target_h, target_w = target_loc
    ...
    for filepath in manifest:
        try:
            ...
            for i in range(2):
                ...
                car_painting = cartoonize_image(painting_path)
                if car_painting is None:
                    continue
                painting = add_border(car_painting) if np.random.randint(0, 2) else car_painting
                ...
                base_quad, target_loc = random_base_quad(img.shape, (p_h, p_w), scale_range=(0.6, 0.85))
                # --- Compute the maximum perturbation based on the miimum depth of background behind destination quad ---
                # Create a mask for the destination quadrilateral.
                poly = base_quad.reshape((4, 1, 2)).astype(np.int32)
                ...
                min_dist_cam = np.min(bg_depths)
                #perturbation = min(1/target_h,1/target_w)*min_dist_cam
                y_conv = 2*min_dist_cam*math.tan(0.8203/2)/img_h
                x_conv = 2*min_dist_cam*math.tan(1.0472/2)/img_w
                target_h_dist = target_loc[0]*y_conv
                target_w_dist = target_loc[1]*x_conv
                #print(target_h_dist,target_w_dist)
                if target_h_dist<min_dist_cam or target_w_dist<min_dist_cam:
                    perturbation = 0.15
                else:
                    perturbation = min(((target_h_dist**2 - min_dist_cam**2)**0.5) /target_h_dist,((target_w_dist**2 - min_dist_cam**2)**0.5) /target_w_dist,0.15)
                #print(min_dist_cam,target_loc,perturbation)

                dest_pts = random_perturb_quad(img.shape, target_loc, base_quad, perturbation)
            ...
            print(f"  Exception with {input_image_path}, file processing cancelled")
