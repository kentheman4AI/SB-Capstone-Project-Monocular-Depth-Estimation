...
def random_left_half_region(img_shape):
    """
    Return a random rectangular region from the left half of the image.
    The region size is chosen between 50%-80% of the left half dimensions.
    Returns (x, y, w, h).
    """
    h, w = img_shape[:2]
    left_half_w = w // 2
    # Random region width and height (as fraction of left half and full height)
    region_w = int(random.uniform(0.5, 0.8) * left_half_w)
    region_h = int(random.uniform(0.5, 0.8) * h)
    # Random top-left such that region fits within left half
    x = random.randint(0, left_half_w - region_w)
    y = random.randint(0, h - region_h)
    return x, y, region_w, region_h

def apply_random_perspective(img_shape,region,base_quad):
    """
...
def process_images_and_depth_maps(datasource,manifest_path,manifest,output_directory):
    ...
    for filepath in manifest:
        ...
        x, y, w_region, h_region = random_left_half_region(img.shape)
        ...
        # Place the mirror on the right half as the symmetric counterpart.
        ...
        dest_pts = apply_random_perspective(img.shape,mirror,base_quad)
